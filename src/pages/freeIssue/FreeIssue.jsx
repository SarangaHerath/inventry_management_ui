...
function createData(id, date, shopName, productName, quantity) {
  return {
    id,
    date,
    shopName,
...
export const FreeIssue = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/v1/freeIssue/getAll"
        );
        ...
        const newRows = responseData.map((data) =>
          createData(
            ...
            data.salesInvoice.shop.shopName,
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(
        `http://localhost:8080/api/v1/sales-invoices/delete/${id}`
      );
  ...
  return (
    <Box sx={{}}>
      ...
      >
        {/* <Link to={"/newsale"}>
          <Button variant="contained">Add New Sales Invoice +</Button>
        </Link> */}
      </div>
